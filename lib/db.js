...
function initTables() {
  ...
  try {
    ...
    if (!hasTrial) {
      ...
      // Существующие тенанты считаются оплаченными бессрочно
      db.run("UPDATE tenants SET paid_until = '2099-12-31T23:59:59'");
...
function createTenant(telegramId, name, leadtehApiToken) {
  ...
  try {
    ...
    const result = run(
      ...
      telegramId, name, leadtehApiToken || '', planId, trialEndsAt
    );
