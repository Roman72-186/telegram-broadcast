...
function getPendingBroadcasts() {
  ...
  const rows = all(
    `SELECT b.*, t.leadteh_api_token
     ...
     AND t.status = 'active'
     AND (t.paid_until >= datetime('now') OR t.trial_ends_at >= datetime('now') OR (SELECT tp.price FROM tariff_plans tp WHERE tp.id = t.tariff_plan_id) = 0)`,
    now
  );
...
function checkSubscription(tenantId) {
  const tenant = get(
    `SELECT t.trial_ends_at, t.paid_until, tp.price as tariff_price
     FROM tenants t
     LEFT JOIN tariff_plans tp ON t.tariff_plan_id = tp.id
     WHERE t.id = ?`,
    tenantId
