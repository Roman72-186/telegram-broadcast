...
// Миграция: увеличить max_contacts в стандартных тарифах (1000→5000, 5000→25000, 50000→100000)
function migrateMaxContacts() {
  try {
    const updated = [];
    const plans = [
      { oldVal: 1000, newVal: 5000, names: ['Стартовый', 'Стартовый + Диалоги'] },
      { oldVal: 5000, newVal: 25000, names: ['Базовый', 'Базовый + Диалоги'] },
      { oldVal: 50000, newVal: 100000, names: ['Профессиональный', 'Профессиональный + Диалоги'] },
    ];
    for (const p of plans) {
      for (const name of p.names) {
        const plan = get('SELECT id, max_contacts FROM tariff_plans WHERE name = ? AND max_contacts = ?', name, p.oldVal);
        if (plan) {
          run('UPDATE tariff_plans SET max_contacts = ? WHERE id = ?', p.newVal, plan.id);
          updated.push(`${name}: ${p.oldVal} → ${p.newVal}`);
        }
      }
    }
    if (updated.length > 0) {
      console.log('[db] Миграция max_contacts:', updated.join(', '));
    }
  } catch (e) {
    console.warn('[db] Миграция max_contacts:', e.message);
  }
}
...
function seedTariffPlans() {
  ...
  if (existing.cnt > 0) {
    // Миграция: добавить тарифы с диалогами если их нет
    const withDialogs = get('SELECT COUNT(*) as cnt FROM tariff_plans WHERE has_dialogs = 1');
    if (withDialogs.cnt === 0) {
      try {
        run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, is_default) VALUES (?, ?, ?, ?, ?, ?)',
          'Стартовый + Диалоги', 1, 30, 5000, 1, 0);
        run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, is_default) VALUES (?, ?, ?, ?, ?, ?)',
          'Базовый + Диалоги', 3, 100, 25000, 1, 0);
        run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, is_default) VALUES (?, ?, ?, ?, ?, ?)',
          'Профессиональный + Диалоги', 10, 500, 100000, 1, 0);
        console.log('[db] Миграция: добавлены тарифы с диалогами');
      } catch (e) {
        console.warn('[db] Миграция тарифов с диалогами:', e.message);
      }
    }
    return;
  }
  ...
  try {
    ...
    run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, price, is_default) VALUES (?, ?, ?, ?, ?, ?, ?)',
      'Стартовый', 1, 30, 5000, 0, 290, 1);
    run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, price, is_default) VALUES (?, ?, ?, ?, ?, ?, ?)',
      'Базовый', 3, 100, 25000, 0, 890, 0);
    run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, price, is_default) VALUES (?, ?, ?, ?, ?, ?, ?)',
      'Профессиональный', 10, 500, 100000, 0, 2490, 0);
    // Рассылки + Диалоги
    run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, price, is_default) VALUES (?, ?, ?, ?, ?, ?, ?)',
      'Стартовый + Диалоги', 1, 30, 5000, 1, 790, 0);
    run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, price, is_default) VALUES (?, ?, ?, ?, ?, ?, ?)',
      'Базовый + Диалоги', 3, 100, 25000, 1, 1390, 0);
    run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, price, is_default) VALUES (?, ?, ?, ?, ?, ?, ?)',
      'Профессиональный + Диалоги', 10, 500, 100000, 1, 2990, 0);
    commit();
  } catch (e) {
    rollback();
    throw e;
...
function hasDialogsAccess(tenantId) {
  const row = get(
    `SELECT tp.has_dialogs FROM tenants t
     LEFT JOIN tariff_plans tp ON t.tariff_plan_id = tp.id
     WHERE t.id = ?`,
    tenantId
  );
  return row?.has_dialogs === 1;
}
