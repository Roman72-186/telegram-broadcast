...
function seedTariffPlans() {
    ...
    if (withDialogs.cnt === 0) {
      try {
        run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, is_default) VALUES (?, ?, ?, ?, ?, ?)',
          'Стартовый + Диалоги', 1, 30, 1000, 1, 0);
        run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, is_default) VALUES (?, ?, ?, ?, ?, ?)',
          'Базовый + Диалоги', 3, 100, 5000, 1, 0);
        run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, is_default) VALUES (?, ?, ?, ?, ?, ?)',
          'Профессиональный + Диалоги', 10, 500, 50000, 1, 0);
  ...
  try {
    ...
    run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, price, is_default) VALUES (?, ?, ?, ?, ?, ?, ?)',
      'Стартовый', 1, 30, 1000, 0, 290, 1);
    run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, price, is_default) VALUES (?, ?, ?, ?, ?, ?, ?)',
      'Базовый', 3, 100, 5000, 0, 890, 0);
    run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, price, is_default) VALUES (?, ?, ?, ?, ?, ?, ?)',
      'Профессиональный', 10, 500, 50000, 0, 2490, 0);
    // Рассылки + Диалоги
    run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, price, is_default) VALUES (?, ?, ?, ?, ?, ?, ?)',
      'Стартовый + Диалоги', 1, 30, 1000, 1, 790, 0);
    run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, price, is_default) VALUES (?, ?, ?, ?, ?, ?, ?)',
      'Базовый + Диалоги', 3, 100, 5000, 1, 1390, 0);
    run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, price, is_default) VALUES (?, ?, ?, ?, ?, ?, ?)',
      'Профессиональный + Диалоги', 10, 500, 50000, 1, 2990, 0);
    commit();
  } catch (e) {
    rollback();
    throw e;
...
function getAllTenants() {
  ...
  return all(
    `SELECT t.*, tp.name as tariff_name, tp.max_contacts as tariff_max_contacts,
...
function checkTariffLimits(tenantId) {
  const tenant = get(
    `SELECT t.*, tp.max_bots, tp.max_contacts
     FROM tenants t
     LEFT JOIN tariff_plans tp ON t.tariff_plan_id = tp.id
     WHERE t.id = ?`,
    tenantId
  ...
  if (!tenant.max_contacts) return { allowed: true };
  ...
  if (uniqueContacts >= tenant.max_contacts) {
    return { allowed: false, reason: `Лимит контактов в месяц исчерпан (${uniqueContacts}/${tenant.max_contacts})` };
  }
