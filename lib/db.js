...
// ============================================
// SQL-обёртки (имитация better-sqlite3 API)
// ============================================
function saveDb() {
  if (!db) return;
...
function saveBroadcast(tenantId, broadcast) {
  ...
  try {
    run(
      `INSERT INTO broadcasts (id, tenant_id, bot_id, name, parse_mode, filters_json, status, scheduled_at, created_by)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      broadcast.id,
      tenantId,
    ...
    (broadcast.messages || []).forEach((msg, i) => {
      run(
        'INSERT INTO broadcast_messages (broadcast_id, sort_order, photo_url, text, buttons_json) VALUES (?, ?, ?, ?, ?)',
        broadcast.id,
        i,
        msg.photo_url || '',
        msg.text || '',
