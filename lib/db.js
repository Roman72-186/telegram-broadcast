...
function createTenant(telegramId, name, leadtehApiToken, status) {
  ...
  try {
    ...
    // Начальный баланс = messages_limit пробного тарифа
    const trialPlan = get("SELECT id, messages_limit FROM tariff_plans WHERE is_default = 1");
    const initialBalance = trialPlan ? trialPlan.messages_limit : 50;
...
function updatePricingConfig(fields) {
  const allowed = ['free_bots', 'price_per_bot', 'free_contacts', 'price_per_100_contacts', 'price_per_100_messages', 'max_bots', 'max_contacts'];
  const sets = [];
  const values = [];
  for (const key of allowed) {
    if (fields[key] !== undefined) {
