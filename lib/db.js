...
function seedTariffPlans() {
  ...
  try {
    // Только рассылки
    run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, is_default) VALUES (?, ?, ?, ?, ?, ?)',
      'Стартовый', 1, 30, 1000, 0, 1);
    run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, is_default) VALUES (?, ?, ?, ?, ?, ?)',
      'Базовый', 3, 100, 5000, 0, 0);
    run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, is_default) VALUES (?, ?, ?, ?, ?, ?)',
      'Профессиональный', 10, 500, 50000, 0, 0);
    // Рассылки + Диалоги
    run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, is_default) VALUES (?, ?, ?, ?, ?, ?)',
      'Стартовый + Диалоги', 1, 30, 1000, 1, 0);
    run('INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs, is_default) VALUES (?, ?, ?, ?, ?, ?)',
...
function createTariffPlan(name, maxBots, maxBroadcasts, maxContacts, hasDialogs) {
  const result = run(
    'INSERT INTO tariff_plans (name, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs) VALUES (?, ?, ?, ?, ?)',
    name, maxBots, maxBroadcasts, maxContacts, hasDialogs ? 1 : 0
  );
  return result.lastInsertRowid;
}

function updateTariffPlan(id, fields) {
  const allowed = ['name', 'max_bots', 'max_broadcasts_per_month', 'max_contacts', 'has_dialogs'];
  const sets = [];
  const values = [];
  for (const key of allowed) {
    if (fields[key] !== undefined) {
