...
function getAllTenants() {
  ...
  return all(
    `SELECT t.*,
     COALESCE(tp.messages_limit, 50) as effective_messages_limit,
     ...
     (SELECT COUNT(*) FROM contact_sends_log WHERE tenant_id = t.id AND sent_at >= ? AND sent_at < ?) as messages_sent
     FROM tenants t
...
function updateTenant(id, fields) {
  ...
  const allowed = ['name', 'leadteh_api_token', 'tariff_plan_id', 'status', 'paid_until', 'trial_ends_at', 'custom_max_bots', 'custom_max_contacts', 'custom_price', 'extra_messages'];
  const sets = [];
  const values = [];
...
function confirmPayment(paymentId) {
  ...
  try {
    ...
    if (payment.tariff_plan_id) {
      run(
        `UPDATE tenants SET tariff_plan_id = ?, updated_at = datetime('now') WHERE id = ?`,
        payment.tariff_plan_id, payment.tenant_id
      );
    }
...
function checkTariffLimits(tenantId) {
  ...
  const messagesLimit = (plan ? plan.messages_limit : 50) + (tenant.extra_messages || 0);

  const month = new Date().toISOString().slice(0, 7);
  const messagesSent = getMessagesSentCount(tenantId, month);
  const botsCount = get(
  ...
  if (messagesSent >= messagesLimit) {
    return { allowed: false, reason: `Лимит сообщений в месяц исчерпан (${messagesSent}/${messagesLimit})` };
  }
