...
function initTables() {
  db.exec(`
    -- Тарифные планы (фиксированные, подсчёт по сообщениям)
    ...
    CREATE TABLE IF NOT EXISTS pricing_config (
      ...
      price_per_100_messages INTEGER NOT NULL DEFAULT 50,
  ...
  // Миграция: messages_limit + sort_order в tariff_plans
  try {
    const tpCols3 = db.exec("PRAGMA table_info(tariff_plans)");
    const hasMsgLimit = tpCols3[0]?.values?.some(row => row[1] === 'messages_limit');
    if (!hasMsgLimit) {
      db.run("ALTER TABLE tariff_plans ADD COLUMN messages_limit INTEGER NOT NULL DEFAULT 50");
      db.run("ALTER TABLE tariff_plans ADD COLUMN sort_order INTEGER NOT NULL DEFAULT 0");
      console.log('[db] Миграция: messages_limit, sort_order добавлены в tariff_plans');
    }
  } catch (e) {
    console.warn('[db] Миграция messages_limit:', e.message);
  }

  // Миграция: price_per_100_messages в pricing_config
  try {
    const pcCols = db.exec("PRAGMA table_info(pricing_config)");
    const hasMsgPrice = pcCols[0]?.values?.some(row => row[1] === 'price_per_100_messages');
    if (!hasMsgPrice) {
      db.run("ALTER TABLE pricing_config ADD COLUMN price_per_100_messages INTEGER NOT NULL DEFAULT 50");
      console.log('[db] Миграция: price_per_100_messages добавлен в pricing_config');
    }
  } catch (e) {
    console.warn('[db] Миграция price_per_100_messages:', e.message);
  }

  // Миграция: free_mode в pricing_config
  try {
    const pcCols2 = db.exec("PRAGMA table_info(pricing_config)");
    const hasFreeMode = pcCols2[0]?.values?.some(row => row[1] === 'free_mode');
    if (!hasFreeMode) {
      db.run("ALTER TABLE pricing_config ADD COLUMN free_mode INTEGER NOT NULL DEFAULT 0");
      console.log('[db] Миграция: free_mode добавлен в pricing_config');
    }
  } catch (e) {
    console.warn('[db] Миграция free_mode:', e.message);
  }

  // Миграция: extra_messages в tenants
  try {
    const tCols2 = db.exec("PRAGMA table_info(tenants)");
    const hasExtraMsg = tCols2[0]?.values?.some(row => row[1] === 'extra_messages');
    if (!hasExtraMsg) {
      db.run("ALTER TABLE tenants ADD COLUMN extra_messages INTEGER NOT NULL DEFAULT 0");
      console.log('[db] Миграция: extra_messages добавлен в tenants');
    }
  } catch (e) {
    console.warn('[db] Миграция extra_messages:', e.message);
  }

  // Миграция: messages_balance в tenants (модель баланса вместо месячных лимитов)
  try {
    const tCols3 = db.exec("PRAGMA table_info(tenants)");
    const hasBalance = tCols3[0]?.values?.some(row => row[1] === 'messages_balance');
    if (!hasBalance) {
      db.run("ALTER TABLE tenants ADD COLUMN messages_balance INTEGER NOT NULL DEFAULT 0");
      // Инициализировать баланс для существующих тенантов: plan.messages_limit + extra_messages
      db.run(`UPDATE tenants SET messages_balance = COALESCE(
        (SELECT tp.messages_limit FROM tariff_plans tp WHERE tp.id = tenants.tariff_plan_id), 50
      ) + COALESCE(extra_messages, 0)`);
      console.log('[db] Миграция: messages_balance добавлен в tenants');
    }
  } catch (e) {
    console.warn('[db] Миграция messages_balance:', e.message);
  }

  // Миграция: пересоздать тарифные планы (фиксированные, по сообщениям)
  try {
    const oldPlans = db.exec("SELECT name FROM tariff_plans WHERE name = 'Пробный'");
    if (!oldPlans[0] || oldPlans[0].values.length === 0) {
      db.run("UPDATE tenants SET tariff_plan_id = NULL WHERE tariff_plan_id IS NOT NULL");
      db.run("UPDATE payments SET tariff_plan_id = NULL WHERE tariff_plan_id IS NOT NULL");
      db.run("DELETE FROM tariff_plans");
      db.run("INSERT INTO tariff_plans (name, messages_limit, price, is_default, sort_order, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs) VALUES ('Пробный', 50, 0, 1, 0, 100, 1000, 50000, 1)");
      db.run("INSERT INTO tariff_plans (name, messages_limit, price, is_default, sort_order, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs) VALUES ('Старт', 500, 390, 0, 1, 100, 1000, 50000, 1)");
      db.run("INSERT INTO tariff_plans (name, messages_limit, price, is_default, sort_order, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs) VALUES ('Бизнес', 3000, 990, 0, 2, 100, 1000, 50000, 1)");
      db.run("INSERT INTO tariff_plans (name, messages_limit, price, is_default, sort_order, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs) VALUES ('Корпоративный', 7000, 1790, 0, 3, 100, 1000, 50000, 1)");
      db.run("INSERT INTO tariff_plans (name, messages_limit, price, is_default, sort_order, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs) VALUES ('Про', 15000, 2990, 0, 4, 100, 1000, 50000, 1)");
      // Привязать существующих тенантов к Пробному
      const trialPlan = get("SELECT id FROM tariff_plans WHERE name = 'Пробный'");
      if (trialPlan) {
        db.run("UPDATE tenants SET tariff_plan_id = ? WHERE tariff_plan_id IS NULL OR tariff_plan_id NOT IN (SELECT id FROM tariff_plans)", trialPlan.id);
      }
      console.log('[db] Миграция: тарифные планы пересозданы (фиксированные, по сообщениям)');
    }
  } catch (e) {
    console.warn('[db] Миграция тарифных планов:', e.message);
  }

  // Миграция: добавить external_id, payment_url, provider, tariff_plan_id в payments
  try {
    ...
    const hasTpId = pCols[0]?.values?.some(row => row[1] === 'tariff_plan_id');
    if (!hasTpId) {
      db.run("ALTER TABLE payments ADD COLUMN tariff_plan_id INTEGER");
      console.log('[db] Миграция: tariff_plan_id добавлен в payments');
    }
  } catch (e) {
    console.warn('[db] Миграция payments (provider):', e.message);
  }

  // Миграция: media_type в broadcast_messages и auto_broadcast_messages
  try {
    const bmCols = db.exec("PRAGMA table_info(broadcast_messages)");
    const hasMediaType = bmCols[0]?.values?.some(row => row[1] === 'media_type');
    if (!hasMediaType) {
      db.run("ALTER TABLE broadcast_messages ADD COLUMN media_type TEXT DEFAULT NULL");
      console.log('[db] Миграция: media_type добавлен в broadcast_messages');
    }
  } catch (e) {
    console.warn('[db] Миграция media_type (broadcast_messages):', e.message);
  }

  try {
    const abmCols = db.exec("PRAGMA table_info(auto_broadcast_messages)");
    const hasAbmMediaType = abmCols[0]?.values?.some(row => row[1] === 'media_type');
    if (!hasAbmMediaType) {
      db.run("ALTER TABLE auto_broadcast_messages ADD COLUMN media_type TEXT DEFAULT NULL");
      console.log('[db] Миграция: media_type добавлен в auto_broadcast_messages');
    }
  } catch (e) {
    console.warn('[db] Миграция media_type (auto_broadcast_messages):', e.message);
  }

  try {
    const chatCols = db.exec("PRAGMA table_info(chats)");
    const hasUsername = chatCols[0]?.values?.some(row => row[1] === 'contact_username');
    if (!hasUsername) {
      db.run("ALTER TABLE chats ADD COLUMN contact_username TEXT DEFAULT ''");
      console.log('[db] Миграция: contact_username добавлен в chats');
    }
  } catch (e) {
    console.warn('[db] Миграция contact_username (chats):', e.message);
  }
}
...
function seedTariffPlans() {
  ...
  beginTransaction();
  try {
    run('INSERT INTO tariff_plans (name, messages_limit, price, is_default, sort_order, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      'Пробный', 50, 0, 1, 0, 100, 1000, 50000, 1);
    run('INSERT INTO tariff_plans (name, messages_limit, price, is_default, sort_order, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      'Старт', 500, 390, 0, 1, 100, 1000, 50000, 1);
    run('INSERT INTO tariff_plans (name, messages_limit, price, is_default, sort_order, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      'Бизнес', 3000, 990, 0, 2, 100, 1000, 50000, 1);
    run('INSERT INTO tariff_plans (name, messages_limit, price, is_default, sort_order, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      'Корпоративный', 7000, 1790, 0, 3, 100, 1000, 50000, 1);
    run('INSERT INTO tariff_plans (name, messages_limit, price, is_default, sort_order, max_bots, max_broadcasts_per_month, max_contacts, has_dialogs) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      'Про', 15000, 2990, 0, 4, 100, 1000, 50000, 1);
    commit();
  } catch (e) {
    rollback();
    throw e;
...
function createTenant(telegramId, name, leadtehApiToken, status) {
  ...
  try {
    ...
    // FREE_MODE: без trial ограничений, безлимитный баланс
    const trialEndsAt = _freeMode
      ? new Date('2099-12-31').toISOString()
      : new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString();
    const trialPlan = get("SELECT id, messages_limit FROM tariff_plans WHERE is_default = 1");
    const initialBalance = _freeMode ? 999999 : (trialPlan ? trialPlan.messages_limit : 50);
    const planId = trialPlan ? trialPlan.id : null;
    const result = run(
      'INSERT INTO tenants (telegram_id, name, leadteh_api_token, trial_ends_at, status, tariff_plan_id, messages_balance) VALUES (?, ?, ?, ?, ?, ?, ?)',
      telegramId, name, leadtehApiToken ? encrypt(leadtehApiToken) : '', trialEndsAt, tenantStatus, planId, initialBalance
    );
...
function ensureSuperAdminTenant(telegramId) {
  const existing = getTenantByTelegramId(String(telegramId));
  if (existing) return existing.id;
  const tenantId = createTenant(String(telegramId), 'Суперадмин', '', 'active');
  updateTenant(tenantId, { messages_balance: 999999 });
  return tenantId;
}
...
function getAllTenants() {
  return all(
    `SELECT t.*,
     COALESCE(tp.name, 'Пробный') as plan_name,
     COALESCE(tp.price, 0) as effective_price,
...
function updateTenant(id, fields) {
  ...
  const allowed = ['name', 'leadteh_api_token', 'tariff_plan_id', 'status', 'paid_until', 'trial_ends_at', 'custom_max_bots', 'custom_max_contacts', 'custom_price', 'extra_messages', 'messages_balance'];
  const sets = [];
  const values = [];
...
function saveBroadcast(tenantId, broadcast) {
  ...
  try {
    ...
    (broadcast.messages || []).forEach((msg, i) => {
      run(
        'INSERT INTO broadcast_messages (broadcast_id, sort_order, photo_url, text, buttons_json, parse_mode, delay_before, media_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        broadcast.id,
        i,
        msg.photo_url || '',
        msg.text || '',
...
function updatePricingConfig(fields) {
  const allowed = ['free_bots', 'price_per_bot', 'free_contacts', 'price_per_100_contacts', 'price_per_100_messages', 'max_bots', 'max_contacts', 'free_mode'];
  const sets = [];
  const values = [];
  for (const key of allowed) {
    if (fields[key] !== undefined) {
...
// --- Платежи ---
function createPayment(tenantId, amount, period, bots, contacts, tariffPlanId) {
  const result = run(
    'INSERT INTO payments (tenant_id, amount, period, bots, contacts, tariff_plan_id) VALUES (?, ?, ?, ?, ?, ?)',
    tenantId, amount, period, bots || 0, contacts || 0, tariffPlanId || null
  );
  return result.lastInsertRowid;
}
...
function confirmPayment(paymentId) {
  ...
  try {
    ...
    if (payment.period === 'extra') {
      // Докупка сообщений — добавить к балансу
      const qty = payment.contacts || 100;
      run(
        `UPDATE tenants SET messages_balance = COALESCE(messages_balance, 0) + ?, updated_at = datetime('now') WHERE id = ?`,
        qty, payment.tenant_id
      );
      commit();
      return { ...payment, status: 'paid', messages_added: qty };
    }

    // Применить тариф к тенанту (tariff_plan_id из платежа)
    if (payment.tariff_plan_id) {
      const plan = get('SELECT messages_limit FROM tariff_plans WHERE id = ?', payment.tariff_plan_id);
      const addMessages = plan ? plan.messages_limit : 0;
      run(
        `UPDATE tenants SET tariff_plan_id = ?, messages_balance = COALESCE(messages_balance, 0) + ?, updated_at = datetime('now') WHERE id = ?`,
        payment.tariff_plan_id, addMessages, payment.tenant_id
      );
    }
...
function logContactSend(tenantId, telegramId, sourceType, sourceId) {
  ...
  // FREE_MODE: не декрементируем баланс
  if (!_freeMode) {
    run('UPDATE tenants SET messages_balance = MAX(0, COALESCE(messages_balance, 0) - 1) WHERE id = ?', tenantId);
  }
}
...
function getMessagesSentCount(tenantId, month) {
  if (!month) month = new Date().toISOString().slice(0, 7);
  const nextMonth = getNextMonth(month);
  const row = get(
    `SELECT COUNT(*) as cnt FROM contact_sends_log
     WHERE tenant_id = ? AND sent_at >= ? AND sent_at < ?`,
    tenantId, month + '-01', nextMonth + '-01'
  );
  return row ? row.cnt : 0;
...
function checkTariffLimits(tenantId) {
  ...
  const plan = tenant.tariff_plan_id ? get('SELECT * FROM tariff_plans WHERE id = ?', tenant.tariff_plan_id) : null;
  const balance = tenant.messages_balance || 0;
  const botsCount = get(
    'SELECT COUNT(*) as cnt FROM bots WHERE tenant_id = ? AND is_active = 1',
    tenantId
  ).cnt;

  // FREE_MODE: все лимиты отключены
  if (_freeMode) {
    return {
      allowed: true,
      limits: {
        plan_name: plan ? plan.name : 'Бесплатный',
        messages_balance: 999999,
  ...
  if (balance <= 0) {
    return { allowed: false, reason: 'Баланс сообщений исчерпан. Докупите сообщения или смените тариф.' };
  }

  return {
    allowed: true,
    limits: {
      plan_name: plan ? plan.name : 'Пробный',
      messages_balance: balance,
      current_bots: botsCount,
    },
  };
}
...
function checkBotLimit(tenantId) {
  // Боты без ограничений на всех тарифах
  return { allowed: true };
}
  ...
  try {
    ...
    for (let si = 0; si < (data.steps || []).length; si++) {
      ...
      for (let mi = 0; mi < (step.messages || []).length; mi++) {
        ...
        run(
          `INSERT INTO auto_broadcast_messages (step_id, sort_order, photo_url, text, buttons_json, parse_mode, media_type)
           VALUES (?, ?, ?, ?, ?, ?, ?)`,
          stepId,
          mi,
          msg.photo_url || '',
...
// --- Чаты (диалоги) ---
function findOrCreateChat(tenantId, botId, contactTelegramId, contactName, contactUsername) {
  const existing = get(
  ...
  if (existing) {
    ...
    const newName = contactName || existing.contact_name || '';
    const newUsername = contactUsername || existing.contact_username || '';
    if (newName !== existing.contact_name || newUsername !== existing.contact_username) {
      run(
        'UPDATE chats SET contact_name = ?, contact_username = ? WHERE id = ?',
        newName, newUsername, existing.id
      );
    }
    return get('SELECT * FROM chats WHERE id = ?', existing.id);
  }

  const result = run(
    'INSERT INTO chats (tenant_id, bot_id, contact_telegram_id, contact_name, contact_username) VALUES (?, ?, ?, ?, ?)',
    tenantId, botId, String(contactTelegramId), contactName || '', contactUsername || ''
  );
...
module.exports = {
  ...
  getPlatformBotUsers,
  // Бесплатный режим
  setFreeMode: (val) => { _freeMode = !!val; },
  isFreeMode: () => _freeMode,
};
