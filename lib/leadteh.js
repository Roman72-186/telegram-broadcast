...
async function fetchListItems(config, schemaId) {
  ...
  const data = await r.json();
  console.log('[DEBUG fetchListItems] raw response keys:', Object.keys(data));
  console.log('[DEBUG fetchListItems] raw response:', JSON.stringify(data, null, 2).slice(0, 2000));
