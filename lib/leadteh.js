...
async function fetchListItems(config, schemaId) {
  ...
  const reqBody = { schema_id: schemaId, bot_id: config.leadtehBotId, api_token: '***' };
  console.log('[DEBUG fetchListItems] request:', JSON.stringify(reqBody));

  if (!r.ok) throw new Error(`Ошибка загрузки элементов списка: ${r.status}`);

  const data = await r.json();
  console.log('[DEBUG fetchListItems] total:', data.meta?.total, 'items:', (data.data || []).length);
  if (Array.isArray(data.data) && data.data.length > 0) {
    console.log('[DEBUG fetchListItems] first item:', JSON.stringify(data.data[0], null, 2));
  }
